...
class UbloxGps(object):
    ...
    def raw_measurements(self):
        """
        Sends a poll request for the RXM class with the RAW Message ID and
        parses ublox messages for the response. The payload is extracted from
        the response which is then passed to the user.

        :return: The payload of the RXM Class and RAW Message ID
        :rtype: namedtuple
        """
        ...
        payload = self.hard_port.readline()
        return payload

    def rtcm_status(self):
        """
        Sends a poll request for the RXM class with the RTCM Message ID and
        parses ublox messages for the response. The payload is extracted from
        the response which is then passed to the user.

        :return: The payload of the RXM Class and RTCM Message ID
        :rtype: namedtuple
        """
        self.send_message(sp.RXM_CLS, self.rxm_ms.get('RTCM'))
        parse_tool = core.Parser([sp.RXM_CLS])
        cls_name, msg_name, payload = parse_tool.receive_from(self.hard_port)
        return payload
